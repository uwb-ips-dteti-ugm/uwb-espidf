...
static void test_phy_sfd_timeout_formula_and_validation(void) {
    ...
    TEST_ASSERT_EQUAL_UINT16(64U, dw3000_hal_phy_preamble_symbols(
        DW3000_PHY_PREAMBLE_LEN_64
    ));
    TEST_ASSERT_EQUAL_UINT8(8U, dw3000_hal_phy_pac_symbols(
        DW3000_PHY_PAC_SIZE_8
    ));
    TEST_ASSERT_EQUAL_UINT8(8U, dw3000_hal_phy_sfd_symbols(
        DW3000_PHY_SFD_TYPE_IEEE_802154Z
    ));
    ...
    TEST_ASSERT_EQUAL(DW3000_ERROR_INVALID_ARG, dw3000_hal_phy_validate_config(
        &config.phy
    ));
