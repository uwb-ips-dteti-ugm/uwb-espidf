...
/* SOFT_RST (0x11:00). Each bit gates one subsystem: 1 = running,
   0 = held in reset. Power-on default is 0xFFFF. Pulse a block by
   clearing its bit and setting it back. */
typedef enum {
    DW3000_PMSC_SOFT_RST_RX   = 1U << 0,
    DW3000_PMSC_SOFT_RST_TX   = 1U << 1,
    DW3000_PMSC_SOFT_RST_HIF  = 1U << 2,
    DW3000_PMSC_SOFT_RST_PMSC = 1U << 3,
    DW3000_PMSC_SOFT_RST_GPIO = 1U << 4,
    DW3000_PMSC_SOFT_RST_BIST = 1U << 5,
    DW3000_PMSC_SOFT_RST_ARM  = 1U << 15,
} dw3000_pmsc_soft_rst_t;

#define DW3000_PMSC_SOFT_RST_ALL_RUN  0xFFFFU
#define DW3000_PMSC_SOFT_RST_ALL_HOLD 0x0000U

/* CLK_CTRL.SYS_CLK (bits [1:0]) / RX_CLK ([3:2]) / TX_CLK ([5:4]) —
   clock-source selector. SYS_CLK is the only one that supports OFF. */
typedef enum {
    DW3000_PMSC_CLK_SRC_AUTO = 0x0U,
    DW3000_PMSC_CLK_SRC_PLL  = 0x1U,
    DW3000_PMSC_CLK_SRC_XTI  = 0x2U,
    DW3000_PMSC_CLK_SRC_OFF  = 0x3U, /* SYS_CLK only */
...
typedef enum {
    ...
    DW3000_PMSC_CLK_GPIO_CLK_EN  = 1U << 22,
    DW3000_PMSC_CLK_GPIO_DCLK_EN = 1U << 23,
    DW3000_PMSC_CLK_GPIO_DRST_N  = 1U << 24,
...
typedef enum {
    DW3000_PMSC_SEQ_AINIT2IDLE = 1U << 8,  /* auto INIT -> IDLE_RC          */
    DW3000_PMSC_SEQ_ATXSLP     = 1U << 9,  /* auto sleep after TX           */
    DW3000_PMSC_SEQ_ARXSLP     = 1U << 10, /* auto sleep after RX           */
    DW3000_PMSC_SEQ_PLL_SYNC   = 1U << 11, /* PLL sync at IDLE_PLL          */
    DW3000_PMSC_SEQ_CIARUNE    = 1U << 12, /* hold CIA running              */
    DW3000_PMSC_SEQ_FORCE2INIT = 1U << 17, /* force FSM back to INIT_RC     */
