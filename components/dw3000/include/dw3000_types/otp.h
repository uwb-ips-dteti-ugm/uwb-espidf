...
typedef enum {
    ...
    DW3000_OTP_CFG_WRITE     = 1U << 6,  /* trigger program         */
    DW3000_OTP_CFG_WRITE_MR  = 1U << 7,  /* write mode register     */
    DW3000_OTP_CFG_DGC_KICK  = 1U << 10, /* DGC tuning load         */
    DW3000_OTP_CFG_LDO_KICK  = 1U << 11, /* LDO trim load           */
    DW3000_OTP_CFG_BIAS_KICK = 1U << 12, /* bias trim load          */
    DW3000_OTP_CFG_OPS_KICK  = 1U << 13, /* OPS table load          */
} dw3000_otp_cfg_flags_t;

/* OTP_CFG.OPS_SEL (bits [15:14]) — selects which OPS block to load on
   OPS_KICK. The exact mapping is channel/PRF-specific; see the user
   manual OTP map. */
typedef enum {
    DW3000_OTP_OPS_SEL_LONG  = 0x0U,
    DW3000_OTP_OPS_SEL_SHORT = 0x1U,
