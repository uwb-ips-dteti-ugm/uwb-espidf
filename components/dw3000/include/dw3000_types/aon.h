...
typedef enum {
    DW3000_AON_DIG_ONW_GO2IDLE = 1U << 0,  /* enter IDLE_RC after wake     */
    DW3000_AON_DIG_ONW_GO2RX   = 1U << 1,  /* start RX after wake          */
    DW3000_AON_DIG_ONW_PGFCAL  = 1U << 8,  /* run PGF cal on wake          */
    DW3000_AON_DIG_ONW_RUN_SAR = 1U << 10, /* sample SAR ADC on wake       */
    DW3000_AON_DIG_ONW_AON_DLD = 1U << 11, /* reload AON RAM into regs     */
...
typedef enum {
    ...
    DW3000_AON_CTRL_SAVE       = 1U << 1, /* copy regs -> AON RAM (sleep) */
    DW3000_AON_CTRL_CFG_UPLOAD = 1U << 2, /* refresh sleep config         */
    DW3000_AON_CTRL_DCA_ENAB   = 1U << 7, /* direct AON access enable     */
} dw3000_aon_ctrl_t;

/* AON_ADDR (0x0A:0C) — 16-bit byte address into the 96-byte AON RAM. */
