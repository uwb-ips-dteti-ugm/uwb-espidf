...
| `tests/hal_pure`            | Build pass          | ESP-IDF Unity app for hardware-free HAL helper tests; validates PHY SFD timeout math, STS SYS_CFG/timestamp quality helpers, GPIO validation, FCMD/TX/RX command classification, TX frame bounds, RX event masks, AES/MAC validation helpers, PMSC/PLL validation helpers, AON/OTP helpers, core defaults/device-ID support, calibration/sync helpers, and CIA/ACC helpers. |
